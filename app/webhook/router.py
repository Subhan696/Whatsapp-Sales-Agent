...
async def ingest_message(
    ...
    customer, created = await crud.get_or_create_customer(db, wa_id, name=name)
    ...
    await crud.upsert_session(db, customer.id, wa_id)
...
async def _process_message_background(
    ...
    try:
        async with factory() as db:
            async with db.begin():
                customer = await ingest_message(db, message, contact)
    ...
    try:
        ...
        async with factory() as db:
            history_rows = await get_conversation_history(db, customer.id, limit=100)
            last_order_ref = await get_latest_cancellable_order_ref(db, customer.id)
            bank_transfer_details = await get_setting(db, "bank_transfer_details")
            business_name = await get_setting(db, "business_name")
            business_description = await get_setting(db, "business_description")
