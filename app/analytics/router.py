...
async def funnel(db: AsyncSession = Depends(get_db)) -> FunnelResponse:
    """Conversion funnel: customer counts per CRM stage with per-stage conversion rates."""
    return await get_funnel(db)


@router.get("/kpis", response_model=KPIsResponse)
async def kpis(db: AsyncSession = Depends(get_db)) -> KPIsResponse:
...
async def customers(
    ...
    return await get_customers_page(db, page=page, per_page=per_page, stage=stage)


@router.get("/products", response_model=ProductPageResponse)
async def products(db: AsyncSession = Depends(get_db)) -> ProductPageResponse:
...
async def orders(
    ...
    return await get_orders_page(db, page=page, per_page=per_page, status=status, payment_method=payment_method)
